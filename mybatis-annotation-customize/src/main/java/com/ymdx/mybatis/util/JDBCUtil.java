...
public final class JDBCUtil {
    ...
    /**
     * 专门检查缓存的连接是否不可以被使用，不可以被使用的话，就返回true
     */
    private static boolean invalid() {
        if (conn != null) {
            ...
            // conn既不是null且也没有关闭，且isValid返回true，说明是可以使用的（返回 false）
            return false;
        } else {
            return true;
        }
    ...
    public static int insert(String SQL, boolean autoGeneratedKeys, List<Object> params) {
        ...
        if (params.size() > 0) {
            ...
            Connection c = null;
            try {
                // 从PreparedStatement对象中获得它对应的连接对象
                c = ps.getConnection();
            } catch (SQLException e) {
                e.printStackTrace();
            }
